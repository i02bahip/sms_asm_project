...
Loop:
    call WaitForLineInterrupt
ContinueFromFrameInterrupt:
    ld a,(Scroll)
    ld b,VDP_HORIZONTAL_SCROLL_REGISTER
...
WaitForLineInterrupt:
           ld a,(VDPStatus)
           bit 6,a
           jp z,WaitForLineInterrupt
           ; TODO cosas que hacer en el line interrupt
           ;-----------------
           ld a,(Scroll)
           ld b,VDP_HORIZONTAL_SCROLL_REGISTER
           call SetRegister      ; Load the vertical scroll register.
           call MoveScroll  ; Update the scroll register buffer.
           ;-----------------
           ; vemos que no se haya llegado al frame interrupt
           bit 7,a ; se ha llegado al line interrupt
           jp z,WaitForLineInterrupt
           res 7,a ; se ha llegado al frame interrupt
           ld (VDPStatus),a
           ret
;==============================================================
; Data
